...
const OfficeStaffDashboard = () => {
  ...
  const handleProcessRequest = async (action) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const payload = { action, comment: actionComment || 'Certificate collected by student.' };
  ...
  // CLIENT-SIDE FILTER: Show only requests that are ready for collection or have been collected
  const actionableRequests = requests.filter(req => 
    ['Ready for Collection', 'Collected'].includes(req.status)
  );

  return (
    <Box>
      <header className="dashboard-header admin-header">
        <Typography variant="h4" component="h2" sx={{color: 'white'}}>Office Staff Panel</Typography>
        <div className="header-right">
          <Clock />
          <div className="header-user-info">
            <span>Welcome, {user?.name}! (Office Staff)</span>
      ...
      <main className="user-management-container">
        <Paper elevation={3}>
          <TableContainer>
            <Typography variant="h5" sx={{ p: 2 }}>Manage Collections</Typography>
            <Table>
              <TableHead>
                <TableRow>
                ...
                {actionableRequests.map((req) => (
                  <TableRow key={req._id}>
                    ...
                    <TableCell align="right">
                      <Button variant="outlined" onClick={() => handleProcessClick(req)}>View Details</Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      {selectedRequest && (
        <Modal open={!!selectedRequest} onClose={() => setSelectedRequest(null)}>
          <Box className="modal-box">
            <Typography variant="h6">Process Collection</Typography>
            ...
            <Typography><b>Current Status:</b> {selectedRequest.status}</Typography>
            <TextareaAutosize
              minRows={3}
            ...
            <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
              ...
              {/* Only show the 'collect' button if the certificate is ready */}
              {selectedRequest.status === 'Ready for Collection' && (
                <Button variant="contained" color="success" onClick={() => handleProcessRequest('collect')}>Mark as Collected</Button>
              )}
            </Box>
          </Box>
        </Modal>
